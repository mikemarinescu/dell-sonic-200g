...
try:
    ...
    from .inf8628 import inf8628Dom
...
XCVR_INTFACE_BULK_WIDTH_QSFP = 128
XCVR_INTFACE_BULK_WIDTH_SFP = 64
XCVR_TYPE_OFFSET = 0
XCVR_TYPE_WIDTH = 1
XCVR_EXT_TYPE_OFFSET = 1
XCVR_EXT_TYPE_WIDTH = 1
XCVR_MEM_TYPE_OFFSET = 2
XCVR_MEM_TYPE_WIDTH = 1
...
OSFP_IPML_MEM_PAGE_OFFSET = ((142 & 0x7f) | 0x100)

OSFP_CMIS_TYPE_ID = "1e"
OSFP_CMIS_TYPE_OFFSET = 0
OSFP_CMIS_VENDOR_NAME_OFFSET = 129
OSFP_CMIS_VENDOR_PN_OFFSET = 148
OSFP_CMIS_HW_REV_OFFSET = 164
OSFP_CMIS_VENDOR_SN_OFFSET = 166
OSFP_CMIS_IPML_MEM_PAGE_OFFSET = ((142 & 0x7f) | 0x100)
...
class SfpUtilBase(object):
    ...
    def _read_eeprom_specific_bytes(self, sysfsfile_eeprom, offset, num_bytes):
        ...
        try:
            ...
            print("Error: reading EEPROM sysfs file %s" % sysfsfile_eeprom.name)
            return None
    ...
    def read_porttab_mappings(self, porttabfile, asic_inst=0):
        ...
        if(parse_fmt_platform_json):
            ...
            if not ports:
                ...
                for intf_name in logical:
                    ...
                    if 'lanes' in ports[intf_name].keys():
                        logical_lanes[intf_name] = ports[intf_name]['lanes']
                    if 'speed' in ports[intf_name].keys():
                        logical_speeds[intf_name] = ports[intf_name]['speed']
                ...
                self.logical_speeds = logical_speeds
                self.logical_lanes = logical_lanes
        ...
        for line in f:
            ...
            if (parse_fmt_port_config_ini):
                ...
                if "speed" in title:
                    port_speed = line.split()[title.index("speed")]
                if "lanes" in title:
                    port_lanes = line.split()[title.index("lanes")]
            ...
            logical_speeds[portname] = port_speed
            logical_lanes[portname] = port_lanes
    ...
    def get_logical_speed(self, logical_port):
        """Returns logical port speed"""

        if logical_port in self.logical_speeds:
            return self.logical_speeds[logical_port]
        else:
            return "N/A"

    def set_logical_speed(self, logical_port, speed):
        """Set logical port speed"""

        if logical_port in self.logical_speeds:
            self.logical_speeds[logical_port] = speed
        else:
            return

    def get_logical_lanes(self, logical_port):
        """Returns logical port lanes"""

        if logical_port in self.logical_speeds:
            return self.logical_lanes[logical_port]
        else:
            return ""

    def _physical_del_logical(self, dict_p2l, port):
        for key in dict_p2l.copy():
            if port in dict_p2l[key]:
                dict_p2l[key].remove(port)
                if len(dict_p2l[key]) == 0:
                    dict_p2l.pop(key)

    def _physical_add_logical(self, dict_p2l, port, index):
        if index in dict_p2l:
            dict_p2l[index].append(port)
        else:
            dict_p2l[index] = [ port ]

    ### add logical port
    def add_logical_port(self, port, index, lanes, speed, asic_inst=0):
        self.logical.append(port)
        self._physical_add_logical(self.physical_to_logical, port, index)
        self.logical_lanes[port] = lanes
        self.logical_speeds[port] = speed
        self.logical_to_physical[port] = [index]
        self.logical_to_asic[port] = asic_inst

        return

    ### delete logical port
    def del_logical_port(self, port):
        self.logical.remove(port)
        self._physical_del_logical(self.physical_to_logical, port)
        self.logical_lanes.pop(port, None)
        self.logical_speeds.pop(port, None)
        self.logical_to_physical.pop(port, None)
        self.logical_to_asic.pop(port, None)
    ...
    def get_eeprom_type(self, port_num):
        type = XCVR_EEPROM_TYPE_UNKNOWN

        raw = self._read_eeprom_devid(port_num, self.IDENTITY_EEPROM_ADDR, 0, 256)
        if raw is None:
            return type

        if (port_num in self.qsfp_ports) or (port_num in self.osfp_ports):
            if raw[0] != raw[128]:
                print("port {}: Warning: Unexpected type id in transceiver EEPROM".format(port_num))
            # QSFPDD check code validation
            if raw[128] in OSFP_TYPE_ID:
                sum = 0
                for i in range(128, 222):
                    sum += int(raw[i], 16)
                if (sum & 0xff) == int(raw[222], 16):
                    type = XCVR_EEPROM_TYPE_OSFP
            # QSFP_CMIS hack (not necessarily guaranteed QSFP56)
            if raw[128] in OSFP_CMIS_TYPE_ID:
                    type = XCVR_EEPROM_TYPE_OSFP56
    ...
    def get_eeprom_dom_raw(self, port_num):
        type = self.get_eeprom_type(port_num)

        if type == XCVR_EEPROM_TYPE_UNKNOWN:
            return None
        elif type == XCVR_EEPROM_TYPE_OSFP:
            return None
    ...
    def get_transceiver_info_dict(self, port_num):
        ...
        info_dict_keys = ['type', 'hardware_rev', 'serial','manufacturer',
                          ...
                          'specification_compliance','type_abbrv_name','vendor_date', 'vendor_oui', 'application_advertisement']
        ...
        # will be implemented when the inf8628 memory map ready
            ...
            eeprom_ifraw = self._read_eeprom_specific_bytes(sysfsfile_eeprom, 0, 256)
            eeprom_page1 = self._read_eeprom_specific_bytes(sysfsfile_eeprom, 256, 128)
            ...
            transceiver_info_dict['type'] = sfp_data['data']['Identifier']
            transceiver_info_dict['type_abbrv_name'] = sfp_data['data']['type_abbrv_name']
            transceiver_info_dict['manufacturer'] = sfp_data['data']['Vendor Name']
            transceiver_info_dict['model'] = sfp_data['data']['Vendor Part Number']
            transceiver_info_dict['hardware_rev'] = sfp_data['data']['Vendor Revision']
            transceiver_info_dict['serial'] = sfp_data['data']['Vendor Serial Number']
            transceiver_info_dict['media_type'] = sfp_data['data']['Media Type']
            transceiver_info_dict['module_state'] = sfp_data['data']['Module State']
            transceiver_info_dict['power_class'] = sfp_data['data']['Power Class']
            transceiver_info_dict['rev_compliance'] = sfp_data['data']['Revision Compliance']
            ...
            if eeprom_page1 is None:
                eeprom_page1 = []
                while len(eeprom_page1) < 128:
                    eeprom_page1.append('00')

            mem_page_data = sfpi_obj.parse_implemented_memory_pages(eeprom_page1, OSFP_IPML_MEM_PAGE_OFFSET & 0x7f)
            ...
            if is_qsfp:
                ...
                raw = self._read_eeprom_specific_bytes(sysfsfile_eeprom, XCVR_MEM_TYPE_OFFSET, XCVR_MEM_TYPE_WIDTH)
                if raw is not None:
                    transceiver_info_dict['memory_type'] = sfpi_obj.decode_upper_memory_type(raw, 0, 1)
            else:
                offset = 0
    ...
    def get_transceiver_dom_info_dict(self, port_num):
        ...
        if type == XCVR_EEPROM_TYPE_UNKNOWN:
            ...
            try:
                dom_raw = []
                while len(dom_raw) < 0x980:
                    dom_raw.append('00')
                dom_pos = 0
                tmp = self._read_eeprom_specific_bytes(sysfsfile_eeprom, dom_pos, 256)
                if tmp is not None:
                    for x in tmp:
                        dom_raw[dom_pos] = x
                        dom_pos += 1
                dom_pos = 0x900
                tmp = self._read_eeprom_specific_bytes(sysfsfile_eeprom, dom_pos, 128)
                if tmp is not None:
                    for x in tmp:
                        dom_raw[dom_pos] = x
                        dom_pos += 1
            ...
            if (qsfp_dom_rev[0:8] == 'SFF-8436' or qsfp_tx_power_support != 'On'):
    ...
    def get_transceiver_dom_threshold_info_dict(self, port_num):
        ...
        if port_num in self.osfp_ports:
            ...
            qsfp_dom_rev_raw = self._read_eeprom_specific_bytes(sysfsfile_eeprom, QSFP_DOM_REV_OFFSET, QSFP_DOM_REV_WIDTH)
            if qsfp_dom_rev_raw is not None:
                qsfp_dom_rev_data = sfpd_obj.parse_sfp_dom_rev(qsfp_dom_rev_raw, 0)
                qsfp_dom_rev = qsfp_dom_rev_data['data']['dom_rev']['value']
            else:
            ...
            qsfp_dom_capability_raw = self._read_eeprom_specific_bytes(sysfsfile_eeprom, (128 + XCVR_DOM_CAPABILITY_OFFSET), XCVR_DOM_CAPABILITY_WIDTH)
            if qsfp_dom_capability_raw is not None:
                sfpi_obj = sff8436InterfaceId()
                if sfpi_obj is None:
                    return transceiver_dom_threshold_info_dict
            ...
            if (qsfp_dom_rev[0:8] != 'SFF-8436' and qsfp_tx_power_support == 'On'):
            ...
            # The SFP DOM support is known to be faulty and incomplete
            # At the time of writting, sff8472Dom.parse_module_monitor_params()
            # is not available on the master branch of community repository, too
            # and thus the following logic shall be skipped to prevent xcvrd
            # from crashing
            return transceiver_dom_threshold_info_dict
    ...
    def get_transceiver_compatibility(self, port_num, port_speed):
        transceiver_info_dict = {}
        compatible = True

        transceiver_info_dict = self.get_transceiver_info_dict(port_num)
        if transceiver_info_dict is None:
            return "eeprom read fail", None

        xcvr_type = transceiver_info_dict['type']
        xcvr_type_br = transceiver_info_dict['type_abbrv_name']
        nbr = transceiver_info_dict['nominal_bit_rate']
        if transceiver_info_dict['specification_compliance'] != 'N/A':
            spec_compliance_dict = eval(transceiver_info_dict['specification_compliance'])
        else:
            spec_compliance_dict = {}
        osfp_speeds = ['400000', '200000', '100000', '50000']

        type = self.get_eeprom_type(port_num)
        if type == XCVR_EEPROM_TYPE_QSFP:
            if xcvr_type_br == "QSFP+":
                if port_speed not in ['40000', '10000']:
                    compatible = False
            elif xcvr_type_br == "QSFP28":
                if port_speed not in ['100000', '25000']:
                    compatible = False
            elif xcvr_type_br == "QSFP-DD":
                if port_speed not in osfp_speeds:
                    compatible = False
            else:
                return xcvr_type_br, None
        elif type == XCVR_EEPROM_TYPE_SFP:
            compliance_codes_1g = ["1X SX", "1X LX", "1X Copper Active", "1X Copper Passive"]
            mode_1g = ""
            if 'InfinibandComplianceCode' in spec_compliance_dict:
                mode_1g = spec_compliance_dict.get('InfinibandComplianceCode')
            #SFP28
            if nbr == '255' and port_speed != '25000':
                xcvr_type_br = str("SFP28")
                compatible = False
            #1G
            elif mode_1g in compliance_codes_1g:
                if port_speed != '1000':
                    xcvr_type_br = str("1G-SFP")
                    compatible = False
            #10G
            elif port_speed != '10000':
                xcvr_type_br = str("SFP/SFP+")
                compatible = False
        elif type == XCVR_EEPROM_TYPE_OSFP:
            if port_speed not in osfp_speeds:
                compatible = False
        else:
            # XCVR_EEPROM_TYPE_UNKNOWN
            return "invalid transceiver type", None

        return xcvr_type_br, compatible

    def is_media_type_set_required(self, port_num):
        """
        :param port_num: Integer, index of physical port
        :returns: Boolean, True if media type set required, False if not
        """
        return None
